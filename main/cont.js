(() => {
    ...
    setTimeOut(async () => {
        userInit();
        Console("查询用户信息中。。。请稍后");
        let res = await _ajax(typeIndex ? "/portal/info/getMyInfo" : "/api/student/stuInfo/getStuInfo"); //查询用户信息
        if (res == null || res.code == -1) {
            ...
            let name = localStorage.getItem("displayName"),
                sId = localStorage.getItem("userName");
            ...
            Console(`[${name}] 用户您好， 欢迎━( * ｀∀´ * ) ノ亻!使用本脚本，该脚本为3.0版本 `);
            if (typeIndex) Console(`该脚本不支持做测验题, 所以会出现课程未完成但没办法全部完成子节点情况，是因为跳过了测验题，建议手动完成测验题再执行该脚本或者忽视测验题`);
            config._Lock = false;
        }
    });

    function isFinish(bln) {
        return config.isLoadFinish || bln;
    }
    ...
    function getStyle() {
        ...
        #hcq-content-left.open~.switch-box.open {left: 180px;z-index: 9}
